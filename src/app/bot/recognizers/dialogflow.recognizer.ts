...
export class DialogFlowRecognizer {
  ...
  private projectId;
  private readonly jsonSimpleValueKinds = new Set([
    'numberValue',
    'stringValue',
    'boolValue',
  ]);
  ...
  public async recognize(turnContext: TurnContext): Promise<IRecognitionResult> {
    ...
    if (utterance) {
      ...
      const sessionId = `${turnContext.activity.from.id}-${turnContext.activity.channelId}`.substring(0, 30);
      try {
        const data = await this.makeRequest(utterance, sessionId, turnContext.activity.locale || 'es');
        return data;
      } catch (error) {
  ...
  private async makeRequest(text: string, sessionId: string, languageCode: string): Promise<IRecognitionResult> {
  ...
  private processDialogFlowData(data: DetectIntentResponse) {
    const result: { entities: { [entityName: string]: any }, intent: string } = { entities: {}, intent: null };
